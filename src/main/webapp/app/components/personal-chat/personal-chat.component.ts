/* eslint-disable */
import { Component, Input, OnChanges, OnDestroy, OnInit, SimpleChanges, ViewChild, ElementRef } from '@angular/core';
import { Stomp } from '@stomp/stompjs';
import io from 'socket.io-client';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { interval, Subscription } from 'rxjs';

@Component({
  selector: 'jhi-personal-chat',
  templateUrl: './personal-chat.component.html',
  styleUrls: ['./personal-chat.component.scss'],
})
export class PersonalChatComponent implements OnInit, OnDestroy, OnChanges {
  profileImg: string =
    'data:image/jpeg;base64,';
  inputValue: string = '';
  socket: any;
  message: string = '';
  messages: string[] = [];
  @Input() selectedItem: any;
  @Input() authToken: string = '';
  sampleChatArr: { name: string; message: string; senderOrReciever: number }[] = [];
  messageArr: any;
  selectedContactDetails: any;
  consoleLogSubscription: Subscription;
  @ViewChild('scrollContainer') scrollContainer!: ElementRef;

  constructor(private http: HttpClient) {
    this.consoleLogSubscription = interval(3000).subscribe(() => {
      this.getChatMessages(this.selectedContactDetails?.id);
    });
  }

  ngOnInit(): void {
    // this.socket = io('http://localhost:8080'); // Replace with your server URL
    // this.socket.on('connection', () => {
    //   console.log('server connected');
    // });
    // this.socket.on('disconnect', () => {
    //   console.log('server disconnected');
    // });
    // this.socket.on('message', (message: string) => {
    //   this.messages.push(message);
    // });
    // this.stompService.subscribe('/topic/vehicle', (item: any) => {
    //   console.log(item);
    // });
  }
  ngOnChanges(changes: SimpleChanges): void {
    if (changes) {
      console.log('myValue changed:', changes.selectedItem.currentValue);
      const details = changes.selectedItem.currentValue;
      this.selectedContactDetails = changes.selectedItem.currentValue;
      this.getChatMessages(details.id);
      console.log(changes.selectedItem.currentValue);
    }
  }

  chatMessaggesAPI(id: string) {
    const token = this.authToken;
    const apiUrl = `http://localhost:8080/api/user-chat-history/${id}`;

    // Headers including the bearer token
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`,
    });
    // Making HTTP GET request with headers
    return this.http.get<any>(apiUrl, { headers: headers });
  }
  sendMessageAPI(msg: string) {
    console.log('🚀 ~ PersonalChatComponent ~ sendMessageAPI ~ msg:', msg);
    const token = this.authToken;
    const apiUrl = `http://localhost:8080/api/user-chat-histories`;

    // Headers including the bearer token
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`,
    });
    // Making HTTP GET request with headers
    return this.http.post<any>(
      apiUrl,
      {
        senderUserID: 1051,
        receiverUserID: this.selectedItem.id,
        messageContent: msg,
      },
      { headers: headers }
    );
  }

  getChatMessages(id: string) {
    this.chatMessaggesAPI(id).subscribe(data => {
      // this.messageArr = data;
      this.messageArr = Array.isArray(data) ? data : [data];
      this.scrollToElement();
    });
  }
  public postMessage(msg: string) {
    console.log('🚀 ~ PersonalChatComponent ~ postMessage ~ msg:', msg);
    this.sendMessageAPI(msg).subscribe(data => {
      // this.messageArr = data;
      this.messageArr = Array.isArray(data) ? data : [data];
      this.scrollToElement();
    });
  }

  onEnterKeyPressed() {
    console.log('Input value:', this.inputValue);
    // this.sampleChatArr.push({ name: 'sample', message: this.inputValue, senderOrReciever: Math.floor(Math.random() * 2) });
    this.postMessage(this.inputValue);
    this.inputValue = '';
  }

  ngOnDestroy(): void {
    // this.socket.on('disconnect', () => {});
    if (this.consoleLogSubscription) {
      this.consoleLogSubscription.unsubscribe();
    }
  }
  handleSelectedItem(item: any) {
    console.log(item);
  }

  scrollToElement(): void {
    const targetElement = document.getElementById('scrollDiv');

    if (targetElement) {
      targetElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    console.log(this.scrollContainer);
    const scrollContainerEl = this.scrollContainer.nativeElement;
    scrollContainerEl.scrollTop = scrollContainerEl.scrollHeight;
  }

  handleClick() {}
}

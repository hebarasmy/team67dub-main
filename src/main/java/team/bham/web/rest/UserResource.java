...
public class UserResource {
    ...
    /**
     * {@code GET /token} : get the authentication token for the current user.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body containing the authentication token, or with status {@code 404 (Not Found)} if the user is not found.
     */
    @GetMapping("/token")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<String> getUserToken() {
        log.debug("REST request to get token for User : {}", SecurityUtils.getCurrentUserLogin().orElse(null));
        Optional<String> currentUserJWT = SecurityUtils.getCurrentUserJWT();
        if (currentUserJWT.isPresent()) {
            String jwtToken = currentUserJWT.get();
            return ResponseEntity.ok(jwtToken);
        } else {
            // Handle the case when the JWT token is not present
